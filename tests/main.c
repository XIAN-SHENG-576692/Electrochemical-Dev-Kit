...
int main( int argc, char *argv[ ] )
{
    ...
    else if (strcmp(argv[1], "dpv") == 0)
    {
        ...
        if(e_end != 400) return -1;
        ENERGY_TYPE e_energy_step = dpv_get_energy_by_index(&dpv, 5, ELECTROCHEMICAL_DPV_SELECTION_STEP);
        if(e_energy_step != 450) return -1;
        ENERGY_TYPE e_energy_pulse = dpv_get_energy_by_index(&dpv, 5, ELECTROCHEMICAL_DPV_SELECTION_PULSE);
        if(e_energy_pulse != 500) return -1;
        ENERGY_TYPE e_energy_all = dpv_get_energy_by_index(&dpv, 5, ELECTROCHEMICAL_DPV_SELECTION_ALL);
        if(e_energy_all != 530) return -1;
        NUM_TYPE n_step_total = dpv_get_n_step_total(&dpv);
        if(n_step_total != 20) return -1;
        RATE_TYPE scan_rate = dpv_get_scan_rate(&dpv);
        if(scan_rate != 1) return -1;
        TIME_TYPE t_interval = dpv_get_t_interval(&dpv);
        if(t_interval != 10) return -1;
        TIME_TYPE t_total = dpv_get_t_total(&dpv);
        if(t_total != 100) return -1;
        TIME_TYPE t_time_step = dpv_get_time_by_index(&dpv, 5, ELECTROCHEMICAL_DPV_SELECTION_STEP);
        if(t_time_step != 50) return -1;
        TIME_TYPE t_time_pulse = dpv_get_time_by_index(&dpv, 5, ELECTROCHEMICAL_DPV_SELECTION_PULSE);
        if(t_time_pulse != 58) return -1;
        TIME_TYPE t_time_all = dpv_get_time_by_index(&dpv, 5, ELECTROCHEMICAL_DPV_SELECTION_ALL);
        if(t_time_all != 28) return -1;
        return 0;
    }
