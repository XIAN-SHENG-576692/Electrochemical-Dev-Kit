 ...
 */
#define DECLARE_ELECTROCHEMICAL_CV(name, energy_type, index_type, num_type, rate_type, time_type) \
    typedef struct { \
...
\
    static inline energy_type name##_get_e_vertex(const name##_params *const p) { \
...
\
    static inline energy_type name##_get_energy_by_index(const name##_params *const p, index_type index) { \
        return p->e_begin + (p->e_step * ((index > p->n_step) ? (2 * p->n_step - index) : index)); \
    } \
\
...
\
    static inline time_type name##_get_t_ramp(const name##_params *const p) { \
